                        ...
                        <table class="max-w-max rounded-md">
                            ...
                            @foreach($teilnehmer as $tn)
                                <tr class="hover:bg-blue-100" wire:key="{{ $tn->web_id }}">
                                    ...
                                    <td class="px-2 py-1 border ">{{ $tn->fullname }}</td>
                                                ...
                                                <ul
                                                    ...
                                                    @foreach($suchErgebnisse as $person)
                                                        <li wire:click="addMember({{ $person->id }},'tn','wl'); open = false; search = ''"
                                                            wire:confirm="{{ $person->fullname }} zu Warteliste hinzufügen?"
                                                            class="p-0 hover:bg-gray-200 cursor-pointer">
                                                            {{ $person->fullname }}
                                                        </li>
                                                    @endforeach
                            ...
                            <table class="max-w-max rounded-md">
                                ...
                                @foreach($wlist as $wl)
                                    <tr class="hover:bg-blue-100" wire:key="{{ $wl->web_id }}">
                                        ...
                                        <td class="px-2 py-1 border ">{{ $wl->fullname }}</td>
                                        <td class="text-center px-3 py-1 border">
                ...
                @if($action->ac_max_guests > 0)
                    <div class="border rounded-lg p-4 shadow-md">
                        ...
                        <div class="mb-2 flex justify-between items-start w-full">
                            ...
                            @else
                                <p class="font-medium">Gästeplätze reserviert: {{ $action->ac_max_guests }}</p>
                            @endif
                            <div x-data="{ open: false }" class="relative" @keydown.escape.window="open = false">
                                <button @click="open = !open" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded">hinzufügen</button>
                                <div x-show="open" @click.away="open = false" class="absolute p-1 mt-2 w-60 bg-white border rounded shadow-lg z-50">
                                    <p class="text-xl font-semibold mb-2">Gast hinzufügen für</p>
                        ...
                        @if (count($guests) > 0)
                            <table class="border border-grey-00 rounded-md border-separate shadow-sm">
                                <thead>
                                <tr class="font-medium bg-gray-200">
                                    <td class="min-w-50 px-2 py-1 border">Mitglied</td>
                                    <td class="min-w-20 px-2 py-1 border">Gast</td>
                                    <td class="min-w-20 px-2 py-1 border">Bezug</td>
                                    <td class="min-w-40 px-3 py-1 border">Status</td>
                                ...
                                @foreach($guests as $gst)
                                    <tr class="hover:bg-blue-100" wire:key="{{ $gst->id }}">
                                        <td class="px-2 py-1 border ">{{ $gst->fullname }}</td>
                                        <td class="px-2 py-1 border ">{{ $gst->gst_name }}</td>
                                        <td class="px-2 py-1 border ">{{ $gst->reference }}</td>
                            ...
                            <div class=" flex flex-row">
                                <button wire:click="saveGuests" class="w-52 px-4 py-2 mt-2 bg-blue-500 text-white hover:bg-blue-700 rounded">Änderungen speichern</button>
                                @if($savedGuests)
                                    <div class="px-4 pt-3 font-bold text-blue-500">Änderungen gespeichert</div>
                                @endif
                            </div>
                            <div class=" flex flex-row">
                                @if($guestsEmailsCount > 0)
                                    <button wire:click="sendEmailsGuests" class="w-52 px-4 py-2 mt-2 bg-blue-500 text-white hover:bg-blue-700 rounded">{{ $guestsEmailsCount }} Emails senden</button>
                                @else
                                    <button class="w-52 px-4 py-2 mt-2 bg-gray-300 text-white rounded">{{ $guestsEmailsCount }} Emails senden</button>
                                @endif
                                @if($sentEmailsGuests)
                                    <div class="w-60 px-4 pt-3 font-bold text-blue-500">{{ $guestsEmailsSent }} Emails gesendet</div>
                                @endif
                            </div>
                        @endif
                    </div>

                ...
                <td class="text-center">
                    <div x-data="{ open: false }" class="relative" @keydown.escape.window="open = false">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute p-1 mt-2 w-52 bg-white border rounded shadow-lg z-50">
                            ...
                            @if(in_array($action->action_state_sc,['of','gs']))
                                <button wire:click="openCancelModal({{ $action->id }})" class="block w-full px-3 py-1 text-gray-700 hover:bg-gray-200">Fahrt absagen</button>
                            @endif
